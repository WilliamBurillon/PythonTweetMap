...
class TweepyInDB:
    ...
    def insertIntoTable(self):
        tweets_file = open(self.tweets_filename, "r",encoding='utf-8')
            ...
            if 'text' in tweet:  # only messages contains 'text' field is a tweet
                try:
                    self.mycursor.execute(
                        """INSERT INTO Tweets (id, nom, text, quoted_status_text, creation_date, position_X, position_Y) VALUES(%(id)s ,%(name)s ,%(text)s ,%(quoted_status_text)s ,%(creation_date)s ,%(position_X)s ,%(position_Y)s)""",
                        dic)
                    self.mydb.commit()
                except:
